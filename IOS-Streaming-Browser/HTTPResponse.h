...
/**
    @brief Gets the length of the HTTP response content length in bytes.
...
/**
    @brief The HTTP server supports range requests in order to allow things like file download resumption and optimized streaming on mobile devices.
    @return UInt64
**/
- (UInt64)offset;
...
/**
    @brief Returns the data for the response.
...
/**
    @brief Should only return YES after the HTTPConnection has read all available data.
...
/**
    If you need time to calculate any part of the HTTP response headers (status code or header fields), this method allows you to delay sending the headers so that you may asynchronously execute the calculations.
    ...
    If you return YES from this method, the HTTPConnection will wait for you to invoke the responseHasAvailableData method.
...
/**
    @brief Status code for response.
...
/**
    @brief If you want to add any extra HTTP headers to the response, simply return them in a dictionary in this method.
    @return NSDictionary
**/
- (NSDictionary *)httpHeaders;

/**
    @brief If you don't know the content-length in advance, implement this method in your custom response class and return YES.
...
/**
    @brief This method is called from the HTTPConnection class when the connection is closed, or when the connection is finished with the response.
    If your response is asynchronous, you should implement this method so you know not to invoke any methods on the HTTPConnection after this method is called (as the connection may be deallocated).
    @return void
**/
 ...
 * 
 * HTTPConnection supports asynchronous responses.  All you have to do in your custom response class is asynchronously generate the response, and invoke HTTPConnection's responseHasAvailableData method.
 * You don't have to wait until you have all of the response ready to invoke this method.  For example, if you generate the response in incremental chunks, you could call responseHasAvailableData after generating each chunk.  Please see the HTTPAsyncFileResponse class for an example of how to do this.
 * 
 ...
 * An example might be the response needs to generate some custom header fields, or perhaps the response needs to look for a resource on network-attached storage.
 ...
 * After returning YES from this method, the HTTPConnection will wait until the response invokes its responseHasAvailableData method. After this occurs, the HTTPConnection will again query the delayResponseHeaders method to see if the response is ready to send the headers.
